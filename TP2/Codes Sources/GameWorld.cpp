	...
	m_bHumanControlled(false)
{
	...
	for (int i = 0; i < Prm.NumLeaders; ++i)
	{
		...
		m_Vehicles.emplace(m_Vehicles.begin(), new LeaderAgent(this,
...
void GameWorld::HandleKeyPresses(WPARAM wParam)
{
	...
	case 'L':
	{
		...
		//determine a random starting position
		Vector2D SpawnPosLeader = Vector2D(m_cxClient / 2.0 + RandomClamped()*m_cxClient / 2.0,
			m_cyClient / 2.0 + RandomClamped()*m_cyClient / 2.0);

		LeaderAgent* newLeader = new LeaderAgent(this,
			SpawnPosLeader,           //initial position
			RandFloat()*TwoPi,        //start rotation
			Vector2D(0, 0),           //velocity
			Prm.VehicleMass,          //mass
